...
It may not be obvious, but the labels that `finboxio/rancher-lb` recognizes and uses to update the HAProxy config take the following form:
...
So to customize your `finboxio/rancher-lb` deployment, you can define options in the service metadata. Any of these options can be omitted, but here's a complete sample of what's supported:
